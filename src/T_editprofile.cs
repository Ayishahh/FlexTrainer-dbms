...
namespace FlexTrainer
{
    public partial class T_editprofile : Form
    {
        ...
        {
            InitializeComponent();
            this.Load += T_editprofile_Load;
        }

        private void T_editprofile_Load(object sender, EventArgs e)
        {
            LoadUserProfile();
        }
        ...
        private void LoadUserProfile()
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(DatabaseHelper.ConnectionString))
                {
                    ...
                    // Get user data including Trainer-specific information
                    string query = @"SELECT u.User_ID, u.Username, u.email, u.Password, u.DOB,
                                    g.Gym_name
                                    FROM Users u
                                    JOIN Works_For wf ON u.User_ID = wf.Trainer_ID
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(DatabaseHelper.ConnectionString))
                {
                    ...
                    string getCurrentQuery = @"SELECT u.First_name, u.Last_name, t.Experience, t.Speciality
                                               FROM Users u
                    ...
                    using (SqlCommand getCmd = new SqlCommand(getCurrentQuery, conn))
                    {
                        ...
                        using (SqlDataReader reader = getCmd.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                experience = reader.IsDBNull(2) ? "" : reader.GetString(2);
                    ...
                    using (SqlCommand cmd = new SqlCommand("SP_Update_Existing_User", conn))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@Specialization", string.IsNullOrWhiteSpace(speciality) ? (object)DBNull.Value : speciality);
